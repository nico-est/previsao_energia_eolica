...
nomes = ['Alvorada', 'Araçás', 'Caetité 123', 'Guirapá', 'Licínio de Almeida', 'Morrão', 'Planaltina']
...
for i in usinas:
    ...
    plt.ylim(bottom = 0)
    plt.xlim(datetime.strptime('2019-01-01 00:00:00', '%Y-%m-%d  %H:%M:%S'),
             datetime.strptime('2021-01-01 00:00:00', '%Y-%m-%d  %H:%M:%S'))
    
    plt.ylabel('Geração (MWmed)')
    plt.savefig('teste_previsao_' + i + '.png')
    #plt.legend()
    plt.show()
