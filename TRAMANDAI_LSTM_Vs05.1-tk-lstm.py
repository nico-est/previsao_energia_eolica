...
arq=os.getcwd()+'\\Setembro'+'\\pares_tramandai_normalizados.csv'
dados_normalizados = pd.read_csv(arq, engine='python',sep=',',encoding='latin-1')


arq=os.getcwd()+'\\Setembro'+'\\pares_tramandai.csv'
...
scaler_atlantica = MinMaxScaler()
scaler_bom_jardim = MinMaxScaler()
scaler_cidreira = MinMaxScaler()
...
dados['geração usina atlantica'] = scaler_atlantica.fit_transform(dados['geração usina atlantica'].values.reshape(-1, 1))

dados['geração usina bom_jardim'] = scaler_bom_jardim.fit_transform(dados['geração usina bom_jardim'].values.reshape(-1, 1))

dados['geração usina cidreira_1'] = scaler_cidreira.fit_transform(dados['geração usina cidreira_1'].values.reshape(-1, 1))


scalers = [scaler_atlantica, scaler_bom_jardim, scaler_cidreira]
...
usinas =  ['geração usina atlantica', 
       'geração usina cidreira_1']

nomes = ['Atlântica', 'Cidreira 1']
...
for i in usinas:
    ...
    mc = ModelCheckpoint('Plot/Mult/Usina-'+ i + '-best_model.h5', monitor='loss', mode='min', verbose=1, save_best_only=True)
