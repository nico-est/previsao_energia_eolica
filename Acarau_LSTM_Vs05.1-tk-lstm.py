...
arq=os.getcwd()+'\\Setembro'+'\\pares_acarau_normalizados.csv'
dados_normalizados = pd.read_csv(arq, engine='python',sep=',',encoding='latin-1')


arq=os.getcwd()+'\\Setembro'+'\\pares_acarau.csv'
...
dados['geração usina papagaios'] = scaler_papagaios.fit_transform(dados['geração usina papagaios'].values.reshape(-1, 1))


scalers = [scaler_papagaios]
...
usinas =  ['geração usina papagaios']
...
for i in usinas:
    ...
    with tf.device('/device:GPU:0'):
        ...
        if True:
            ...
            history = b_model.fit(trainX, trainY, validation_data=(valX,valY), epochs=NUM_EPOCHS, batch_size=16, verbose=1, callbacks=[es,mc]) #inserir mc
