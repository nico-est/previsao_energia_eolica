...
arq=os.getcwd()+'\\Setembro'+'\\pares_macau_normalizados.csv'
dados_normalizados = pd.read_csv(arq, engine='python',sep=',',encoding='latin-1')


arq=os.getcwd()+'\\Setembro'+'\\pares_macau.csv'
...
scaler_alegria_1 = MinMaxScaler()
scaler_alegria_2 = MinMaxScaler()
scaler_miassaba = MinMaxScaler()
...
dados['geração usina alegria_1'] = scaler_alegria_1.fit_transform(dados['geração usina alegria_1'].values.reshape(-1, 1))

dados['geração usina alegria_2'] = scaler_alegria_2.fit_transform(dados['geração usina alegria_2'].values.reshape(-1, 1))

dados['geração usina miassaba_3'] = scaler_miassaba.fit_transform(dados['geração usina miassaba_3'].values.reshape(-1, 1))


scalers = [scaler_alegria_1, scaler_alegria_2, scaler_miassaba]
...
dados_aux = dados.reset_index()

pos_train1=dados_aux[dados_aux['Tempo'] == Data_Ini_Train].index.values[0]
pos_train2=dados_aux[dados_aux['Tempo'] == Data_Fim_Train].index.values[0]

pos_val1=dados_aux[dados_aux['Tempo'] == Data_Ini_val].index.values[0]
pos_val2=dados_aux[dados_aux['Tempo'] == Data_Fim_val].index.values[0]
...
pos_test1=dados_aux[dados_aux['Tempo'] == Data_Ini_Test].index.values[0]
pos_test2=dados_aux[dados_aux['Tempo'] == Data_Fim_Test].index.values[0]
...
usinas =  ['geração usina alegria_1', 'geração usina alegria_2',
       'geração usina miassaba_3']

usinas =  ['geração usina alegria_1', 'geração usina alegria_2', 'geração usina miassaba_3']

nomes = ['Alegria 1', 'Alegria 2', 'Miassaba 3']
...
for i in usinas:
    ...
    mc = ModelCheckpoint('Plot/Mult/Usina-'+ i + '-best_model.h5', 'loss', mode='min', verbose=1, save_best_only=True)
    ...
    with tf.device('/device:GPU:0'):
        ...
        if True:
            ...
            history = b_model.fit(trainX, trainY, validation_data=(valX,valY), epochs=NUM_EPOCHS, batch_size=8, verbose=1, callbacks=[es,mc]) #inserir mc
    ...
    with open('output_' + i +'.txt', 'w') as f:
        f.write('Average LOSS: Train %.4f, Val %.4f, Test %.4f' % (average(loss_train), average(loss_val), average(loss_test)))
        f.write('Average MSE: Train %.4f, Val %.4f, Test %.4f' % (average(mse_train), average(mse_val), average(mse_test)))
        f.write('Average MAPE: Train %.4f, Val %.4f, Test %.4f' % (average(mape_train), average(mape_val), average(mape_test)))
        f.write('Average MAE: Train %.4f, Val %.4f, Test %.4f' % (average(mae_train), average(mae_val), average(mae_test)))
        f.write('Average R2: Train %.4f, Val %.4f, Test %.4f' % (average(r2_train), average(r2_val), average(r2_test)))
        f.write('Average RMSE: Train %.4f, Val %.4f, Test %.4f' % (average(rmse_train), average(rmse_val), average(rmse_test)))           
    ...
    # Criar um gráfico de dispersão com a diagonal
    plt.figure(figsize=(8, 4))
    
    # Gráfico de dispersão
    plt.scatter(testY, testPredict, color='blue', alpha=0.5, label='Teste')
    
    # Adicionar a diagonal
    plt.plot([min(testY), max(testY)], [min(testY), max(testY)], color='red', linewidth=2, linestyle='--', label='Diagonal')
    
    plt.ylim(bottom=0)
    plt.xlim(0, max(testY))
    plt.xlabel('Real')
    plt.ylabel('Previsto')
    plt.title('Gráfico de Dispersão - Usina ' + nomes[cont])
    plt.savefig('dispersao_' + i + '.png')
    plt.show()
    ...
    plt.title('Treino')
    plt.legend()
    plt.savefig('Plot/treino-' + i + '.png', bbox_inches='tight')
    plt.show()
    ...
    plt.plot(tempo_val, valY, linewidth = 0.1, color = 'skyblue')
    plt.plot(tempo_val, valPredict, label = 'Validação', linewidth = 0.1, color = 'lightcoral')
    plt.plot(tempo_teste, testY, linewidth = 0.1, color = 'skyblue')
    ...
    plt.savefig('Plot/Usina-loss-'+ i + '.png', bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('Plot/Usina-mse-'+ i + '.png', bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('Plot/Usina-mape-'+ i + '.png', bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('Plot/Usina-mae-'+ i + '.png', bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('Plot/Usina-r2-'+ i + '.png', bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('Plot/Usina-rmse-'+ i + '.png', bbox_inches='tight')
    plt.show()
